 ...
 *
 *  For the far-field expansion sum_{i=1}^{p} ( b_m (x_i, x_*) S_m(y,x_*) ) using Cartesian coordinates
 *  the powers and factorial (1/alpha!) (x_b - z_b)^{alpha} are the coefficients b_m and the derivatives
 *  D^{alpha} f(z_b - x_a) are the powers S_m.  The notation is x_i = x_a is the source point, x^s_* = z_a is
 *  the source center, and y = x_b is the target point.  The far-field expansion is the Taylor series
 *
 ...
 *
 *  For a near field series we replace z_a with z_b (the target cell center) and swap x_a and x_b
 *
 ...
 * -- levels for refinements above                                                         c - z_b,c - target cell center
 *                                                                                         s - source = x[0] = z_a
 ...
 *     of the class Potential in Potential.cc by performing an s-expansion approximation to be compared
 *     to the direct calculation.  The Potential.cc member functions SVector returns the powers of the
 *     far-field series as a matrix and the member function getSCoeff returns the coefficients of the
 *     far-field approximation as a matrix.  The corresponding elements of the two matrices are multiplied
 *     together to form the s-expansion and this far-field approximation is compared with the direct calculation
 *     for accuracy.
 *
...
int main()
{
  ...
  std::vector<std::vector<std::complex<double> > > y_SVec = potential.getSVector(y[0].getCoord(), center_xBox.getCoord());

  for (unsigned int i=0; i<=abs_alpha; ++i)
    for (unsigned int j=0; j<=abs_alpha; ++j)
    {
//      std::cout << "We are here" << std::endl;
      s_expansion = s_expansion + y_SCoeff[j][i] * y_SVec[j][i];
    }
