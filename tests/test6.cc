...
int main()
{
  ...
  std::complex<double> SRr_expansion = 0.0;
  std::complex<double> RRr_expansion = 0.0;
  std::complex<double> SSs_expansion = 0.0;
  ...
  std::vector<std::vector<std::complex<double> > > y_SCoeff_old = potential.getSCoeff(x[0].getCoord(), center_xBox.getCoord());
  std::vector<std::vector<std::complex<double> > > x_RCoeff_old = potential.getRCoeff(x[0].getCoord(), center_yBoxParent.getCoord());

  std::vector<std::vector<std::complex<double> > > SSCoeffTranslated = potential.getSS(center_xBox.getCoord(), center_xBoxParent.getCoord(),
 		                                                                               y_SCoeff_old);
  std::vector<std::vector<std::complex<double> > > SRCoeffTranslated = potential.getSR(center_xBox.getCoord(), center_yBox.getCoord(),
 		                                                                              y_SCoeff_old);
  std::vector<std::vector<std::complex<double> > > RRCoeffTranslated = potential.getRR(center_yBoxParent.getCoord(), center_yBox.getCoord(),
 		                                                                              x_RCoeff_old);
  ...
  for (unsigned int i=0; i<4; ++i)
    for (unsigned int j=0; j<4; ++j)
      std::cout << "RRCoeffTranslated[" << i << "][" << j << "] = "
                << RRCoeffTranslated[i][j] << std::endl;
  ...
  for (unsigned int i=0; i<4; ++i)
    for (unsigned int j=0; j<4; ++j)
      std::cout << "SSCoeffTranslated[" << i << "][" << j << "] = "
                << SSCoeffTranslated[i][j] << std::endl;
  ...
  for (unsigned int i=0; i<=abs_alpha; ++i)
    for (unsigned int j=0; j<=abs_alpha; ++j)
    {
      SRr_expansion = SRr_expansion + SRCoeffTranslated[j][i] * y_RVec[j][i];
      RRr_expansion = RRr_expansion + RRCoeffTranslated[j][i] * x_RVecChild[j][i];
      SSs_expansion = SSs_expansion + SSCoeffTranslated[j][i] * y_SVec[j][i];
    }

  std::cout << "SRr_expansion = " << SRr_expansion.real() << " + i" << SRr_expansion.imag() << std::endl;
  std::cout << "RRr_expansion = " << RRr_expansion.real() << " + i" << RRr_expansion.imag() << std::endl;
  std::cout << "SSs_expansion = " << SSs_expansion.real() << " + i" << SSs_expansion.imag() << std::endl;
